...
| [`%package`](xref:microsoft.quantum.iqsharp.magic-ref.package) | Bietet die Möglichkeit, ein NuGet-Paket zu laden. Das Paket muss in der Liste der NuGet-Quellen verfügbar sein. Dies schließt in der Regel „nuget.org“ ein. |
...
| [`%who`](xref:microsoft.quantum.iqsharp.magic-ref.who) | Stellt Aktionen im Zusammenhang mit dem aktuellen Arbeitsbereich bereit. |
| [`%workspace`](xref:microsoft.quantum.iqsharp.magic-ref.workspace) | Gibt eine Liste aller Vorgänge und Funktionen zurück, die in der aktuellen Sitzung definiert sind, entweder interaktiv oder aus dem aktuellen Arbeitsbereich geladen. |
