...
$ $ \mathrm{CNOT}_{01 } (H \otimes 1) \ket{00 } = \bruchteil {1 } {\sqrt{2} } \left (\ket{00 } + \ket{11 } \right), $ $
...
Gemäß der Konvention ist die oberste Zeile Qubit-Register $0, $ und der Rest wird als sequenziell bezeichnet. Die obige Beispiel Verbindung wird als agieren auf zwei Qubits (oder gleichmäßig zwei Register, bestehend aus einem Qubit) dargestellt.
...
Als Verdeutlichung können wir eine zwei-Qubit-einheitliche Operation definieren, $B $ $ (H S \otimes X) $ ist, und die Leitung gleichwertig Ausdrücken.

<!--- ![](.\media\4.svg) --->
<!-- Can't find a way to easily center this... probably an extension needed:  -->
![Leitungs Diagramm eines zwei-Qubit-einheitlichen Vorgangs](~/media/4.svg)

Wir können $B auch anzeigen, $ dass eine Aktion in einem einzigen zwei-Qubit-Register statt 2 1-Qubit registriert ist, je nachdem, in welchem Kontext die Verbindung verwendet wird. Vielleicht ist die nützlichste Eigenschaft von solchen abstrakten Verbindungs Diagrammen, dass Sie komplizierte Quantum-Algorithmen auf hoher Ebene beschreiben können, ohne Sie in grundlegende Gates kompilieren zu müssen.
...
Die Aktion eines Quorums einzeln gesteuerten Tors, die $ \lambda (G) $, wenn der Wert eines einzelnen Qubits die Anwendung von $G steuert $ , kann im folgenden Beispiel der Product State Input $ \lambda (G) (\alpha \ket{0 } + \beta \ket{1 } ) \ket { \psi } = \alpha \ket{0\ket } { \psi } + \beta \ket{1G } \ket { \psi } $ betrachtet werden.
Dies heißt, dass das gesteuerte Gate $G $ auf das Register, das $ \psi enthält, nur dann anwendet, wenn $ das Steuerelement Qubit den Wert $1 annimmt $ .
...
In den Sonderfällen, in denen $G = X $ und $G = Z $ ist, wird die folgende Notation eingeführt, um die kontrollierte Version der Gates zu beschreiben (Beachten Sie, dass das gesteuerte X-Gate das [$CNOT $ Gate](xref:microsoft.quantum.intrinsic.cnot)ist):
...
bietet ein klassisch kontrolliertes Gate, bei dem $G $ auf das klassische Steuer Bit angewendet wird, das Wert $1 ist $ .
