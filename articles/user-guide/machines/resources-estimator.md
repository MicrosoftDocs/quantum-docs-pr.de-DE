...
|__Depth__|Die Tiefe der vom Vorgang durchgeführten Quantum-Verbindung Q# (siehe [unten](#depth-width-and-qubitcount)). Standardmäßig zählt die Tiefe Metrik nur `T` Gates. Weitere Informationen finden Sie unter [tiefen Counter](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).   |
...
__Tiefe:__ Für den Stamm Vorgang, der für die Ausführung benötigt wird, wenn konfigurierte Gate-Zeiten angenommen werden.
...
__Optimizetiefe = false:__ Dies ist der Standardmodus. Qubitmanager wird empfohlen, Qubits wiederzuverwenden und veröffentlichte Qubits wiederzuverwenden, bevor neue zugeordnet werden. Die __Breite__ des Stamm Vorgangs wird zur minimalen Breite (untere Grenze). Es wird eine kompatible __Tiefe__ gemeldet, für die Sie erreicht werden kann. __Qubitcount__ ist die gleiche wie die __Breite__ für den Stamm Vorgang, vorausgesetzt, dass keine Kreditaufnahme erfolgt.

__Optimizetiefe = true:__ Bei qubitmanager wird von der Qubit-Wiederverwendung abgeraten, und die heuristische Optimierung für die Wiederverwendung von Qubit wird während und nach der Ausführung durchgeführt. Die __Tiefe__ des Stamm Vorgangs wird zur minimalen Tiefe (untere Grenze). Für diese Tiefe wird eine kompatible __Breite__ gemeldet (beide können gleichzeitig erreicht werden). Zur Optimierung der Breite können Gates, die später im Programm auftreten, möglicherweise vor den Gates im Programm geplant werden, aber Qubits werden so geplant, dass Sie in einer Weise wieder verwendet werden, dass die Tiefe minimal bleibt. Da Qubits auf der Grundlage von Zeitwerten wieder verwendet werden, wird empfohlen, dass die Gate-Zeiten als ganzzahlige Werte konfiguriert werden. Es ist nicht garantiert, dass die __Breite__ optimal ist. Weitere Informationen finden Sie im Whitepaper [Breite und Tiefe in der](https://github.com/microsoft/qsharp-runtime/tree/main/src/Simulation/Simulators/QCTraceSimulator/Docs)Überwachung.
