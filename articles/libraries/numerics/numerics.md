...
1. **`LittleEndian`**: Ein Qubit- `qArr : Qubit[]` Array, das eine Ganzzahl darstellt, wobei `qArr[0]` das unwichtigste Bit bezeichnet.
...
1. **`FixedPoint`**: Stellt eine reelle Zahl dar, die aus einem `qArr2 : Qubit[]` Qubit-Array und `pos`einer binären Punktposition besteht, die die Anzahl der binären Ziffern links vom binären Punkt zählt. `qArr2`wird auf die gleiche Weise wie `SignedLittleEndian`gespeichert.
...
Zum Auswerten von Smooth-Funktionen wie z. b. $ \sin (x) $ auf einem Quantum-Computer, `FixedPoint` bei dem $x $ eine Quantum-Zahl ist, stellt die- `EvaluatePolynomialFxP` Numerics-Bibliothek des Quantum Development Kits die Vorgänge und `Evaluate[Even/Odd]PolynomialFxP`bereit.

Mit dem ersten `EvaluatePolynomialFxP`,, kann ein Polynoms der Form $ $ P (x) = a_0 + a_1x + a_2x ^ 2 + \cdots + a_dx ^ d, $ $ ausgewertet werden, wobei $d $ den *Grad*angibt. Dazu benötigen `[a_0,..., a_d]` Sie lediglich die Polynomen (vom Typ `Double[]`), die Eingabe `x : FixedPoint` und die Ausgabe `y : FixedPoint` (anfänglich NULL):
```qsharp
...
In einem `cd` der Beispiel Ordner, und führen Sie das Beispiel über aus.
