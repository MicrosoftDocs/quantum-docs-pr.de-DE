...
Das zusätzliche Qubit, das als Steuerelement für `U(m)` verwendet wird, wird dann in $X $ gemessen, um ein einzelnes klassisches `Result`zu erhalten.
...
Um zu verstehen, wie dieser Bayessche Rückschluss Prozess funktioniert, sollten Sie den Fall der Verarbeitung eines einzelnen `Zero` Ergebnisses in Erwägung gezogen.
Beachten Sie, dass $X = \ket{+} \bra{+}-\ket{-}\bra{-}$, sodass $ \ket{+} $ der einzige positive eigen Zustand $X $ ist, der `Zero`entspricht.
Die Wahrscheinlichkeit, `Zero` für eine [`PauliX` Messung](xref:microsoft.quantum.concepts.pauli) auf dem ersten Qubit mit einem Eingabe Zustand von $ \ket{\psi}\ket{\phi} $ zu beobachten, ist folglich \begin{Equation} \pr (\texttt{Zero} | \psi) = \left | \braket{+ | \psi} \right | ^ 2.
...
Wir können auch aus dem oben verwendeten Oracle-Modell verallgemeinern, um fortlaufende Oracles zu ermöglichen, die von der <xref:microsoft.quantum.oracles.continuousoracle>des kanonischen Typs modelliert werden.
...
Der im nächsten Abschnitt erörterte <xref:microsoft.quantum.characterization.robustphaseestimation> Vorgang hat z. b. die folgende Signatur:

```qsharp
operation RobustPhaseEstimation(bitsPrecision : Int, oracle : DiscreteOracle, eigenstate : Qubit[])  : Double
```

Die `bitsPrecision` Eingabe ist für `RobustPhaseEstimation`eindeutig, während `oracle` und `eigenstate` gemeinsam sind.
So kann ein Vorgang, wie in **H2Sample**gezeigt, einen iterativen Phasen Schätz Algorithmus mit einer Eingabe der Form `(DiscreteOracle, Qubit[]) => Unit` akzeptieren, damit ein Benutzer beliebige Phasen Schätz Algorithmen angeben kann:
